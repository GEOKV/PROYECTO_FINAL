...
public:
    ...
    void leer() {
        ...
        cn.abrir_conexion();
        if (cn.getConectar()) {
            string consulta = "SELECT c.idcompra,c.no_orden_compra,c.fecha_orden,c.fechaingreso,p.idproveedor,p.proveedor,p.nit from compras c JOIN proveedores p ON c.idproveedor = p.idproveedor; ";
            const char* x = consulta.c_str();
            q_estado = mysql_query(cn.getConectar(), x);
            if (!q_estado) {
                resultado = mysql_store_result(cn.getConectar());
            ...
            string consulta2 = "SELECT cd.idcompras_detalle, cd.idcompra, cd.idproducto, cd.cantidad, cd.precio_costo_unitario,(cd.cantidad*cd.precio_costo_unitario) AS total, c.idcompra, c.no_orden_compra, p.idProducto, p.producto FROM compras_detalle cd JOIN compras c ON cd.idcompra = c.idcompra JOIN productos p ON cd.idproducto = p.idProducto;";
    ...
    void eliminar() {
        ...
        if (cn.getConectar()) {
            ...
            if (!q_estado) {
                cout << "Eliminacion exitosa de la tabla compras_detalle" << endl;
            }
            else {
            ...
            if (!q_estado) {
                cout << "Eliminacion exitosa de la tabla compras" << endl;
            }
            else {
