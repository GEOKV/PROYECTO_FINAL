...
public:
	...
	void leer() {
		...
		cn.abrir_conexion();
		if (cn.getConectar()) {
			string consulta = "SELECT idCliente,nombres,apellidos,NIT,telefono,correo_electronico,fecha_ingreso, CASE WHEN genero = 0 THEN 'masculino' WHEN genero = 1 THEN 'femenino' END AS genero FROM clientes";
