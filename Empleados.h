...
public:
	...
	void crear() {
		...
		if (cn.getConectar()) {
			...
			string insert = "INSERT INTO empleados(nombres,apellidos,direccion,telefono,DPI,genero,fecha_nacimiento,idPuesto,fecha_inicio_labores,fecha_ingreso) "
	...
	void leer() {
		...
		cn.abrir_conexion();
		if (cn.getConectar()) {
			string consulta = "SELECT e.idEmpleado, e.nombres, e.apellidos, e.direccion, e.telefono, e.DPI, e.fecha_nacimiento, e.fecha_inicio_labores, e.fecha_ingreso, p.puesto, CASE WHEN e.genero = 0 THEN 'masculino' WHEN e.genero = 1 THEN 'femenino' END AS genero FROM empleados AS e INNER JOIN puestos AS p ON e.idPuesto = p.idPuesto";
