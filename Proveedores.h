...
public:
	...
	//set
	void set_idProveedor(int id) { idProveedor = id; }
	...
	void actualizar() {
		...
		if (cn.getConectar()) {
			string t = to_string(idProveedor);
			string update = "UPDATE proveedores SET proveedor='" + proveedor + "',NIT='" + nit + "',Direccion='" + direccion + "',telefono=" + telefono + "' WHERE idProveedor = '" + t + "'";
	...
	void eliminar() {
		...
		cn.abrir_conexion();
		if (cn.getConectar()) {
			string deleteQuery = "DELETE FROM proveedores WHERE idProveedor = '" + to_string(idProveedor) + "'";
